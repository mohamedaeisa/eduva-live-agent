                    ...
                    <React.Fragment key={level.num}>
                        ...
                        <div className="flex flex-col items-center gap-0 relative z-10">
                            ...
                            <div className={`
                                ...
                                ${isActive ? `bg-indigo-600 scale-110 shadow-sm shadow-indigo-500/30` : ''}
...
const AdaptiveQuizModuleV2: React.FC<AdaptiveQuizModuleProps> = ({ user, appLanguage, onBack, onComplete, initialRequest, setView }) => {
    ...
    const [levelGoalCount, setLevelGoalCount] = useState(3);
    ...
    useEffect(() => {
        const checkForResumableSession = async () => {
            ...
            try {
                ...
                // Check if snapshot is valid and recent (<1 hour)
                if (snapshot && (Date.now() - snapshot.timestamp < 3600000)) {
                    const data = snapshot.data;
                    const session = data.session;

                    // Check if there are questions left
                    const nextQ = getNextQuestionSync(session);
                    if (nextQ && session.history && session.history.length < 15) {
                        // Valid resumable session found
                        const questionsLeft = 15 - session.history.length;
                        const subject = data.sessionContext?.subject || session.subject || 'Unknown';
                        const createdAt = data.sessionContext?.startedAt || snapshot.timestamp;
                        ...
                        logger.quiz(`Resumable session found: ${subject} - ${questionsLeft} questions remaining`);
                        return;
                    }
                }

                // If we get here, snapshot is invalid/old/finished
                localStorage.removeItem(RECOVERY_KEY);
                setResumableSession(null);
            } catch (e) {
    ...
    const renderPolymorphicInput = () => {
        ...
        if (currentQ.questionType === 'FillIn') {
            const hasInlineBlank = currentQ.stem.includes('__');
                    ...
                    phase === 'PICKER' && (
                        <div className="space-y-6 max-w-xl mx-auto w-full">
                            ...
                            <Card className="p-4 space-y-3">
                                    ...
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                className="text-[10px] p-1.5 pl-6 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-indigo-500 bg-slate-50 dark:bg-slate-800 dark:text-slate-200 w-32 shadow-sm transition-all focus:w-40 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900"
                                            />
                                ...
                                <section className="space-y-1.5">
                                    <div className="flex justify-between items-center mb-2">
                                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest"><span className="text-indigo-400">03.</span> {t.quiz.setup?.questionsLevel || "QUESTIONS/LEVEL"}</label>
                                        <span className="text-[10px] font-black text-indigo-600">{levelGoalCount} Q's</span>
                                    </div>
                                    <input type="range" min="3" max="9" step="3" value={levelGoalCount} onChange={e => setLevelGoalCount(parseInt(e.target.value))} className="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                                ...
                                <section className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-2"><span className="text-indigo-400">04.</span> {t.quiz.setup?.questionTypes || "QUESTION TYPES"}</label>
                                ...
                                <div className="flex items-center gap-3 sm:gap-4 md:gap-6 justify-center flex-1">
                                    <div className="text-center">
                                        ...
                                        <div className="text-xs sm:text-sm font-black text-indigo-600">
                                            {session.history.length}<span className="text-slate-300">/</span>{session.config.levelQuestionCount * 3}
                                        </div>
                                    </div>
                                    <div className="text-center">
                                    ...
                                    <div className="text-center">
                                        ...
                                        <div className="text-xs sm:text-sm font-black text-amber-600">
                                            {session.pools[1].length + session.pools[2].length + session.pools[3].length}
                                ...
                                <h3 className={`text-xl font-bold leading-snug ${session.subject?.toLowerCase().includes('arabic') || session.subject?.toLowerCase().includes('عربي') ? 'text-right' : 'text-left'}`}>
                                    {(() => {
                                        if (currentQ.questionType === 'FillIn') {
                                            ...
                                            const hasInlineBlank = currentQ.stem.includes('__');
                                            const parts = hasInlineBlank ? currentQ.stem.split(/_+/) : [currentQ.stem];
                                                        ...
                                                        {hasInlineBlank ? parts.map((part, i) => (
                                                                ...
                                                                {i < parts.length - 1 && (
                                                                    <div className="inline-block relative">
                                                                        <input
                                                                            ...
                                                                            value={textInput}
                                                                            readOnly
                                                                            className={`mx-2 bg-transparent border-b-4 ${isAnswered ? (textInput.toLowerCase().trim() === currentQ.answer?.toLowerCase().trim() ? 'border-green-500 text-green-700' : 'border-red-500 text-red-500 line-through') : 'border-indigo-500 text-indigo-700'} font-black text-center outline-none transition-all px-2 min-w-[100px] cursor-pointer`}
                                                                            style={{ width: `${Math.max(100, textInput.length * 20)}px` }}
                                                                            placeholder="???"
                                                                            onClick={() => !isAnswered && setTextInput('')}
                                                                        />
                                                                        {!isAnswered && !textInput && <span className="absolute inset-0 flex items-center justify-center animate-pulse opacity-20 pointer-events-none text-indigo-400">?</span>}
                                                                    </div>
                                                                )}
                                                            </React.Fragment>
                                                        )) : (
                                                            <div className="text-center">
                                                                ...
                                                                <div className="grid grid-cols-2 gap-3">
                                                                    <div className="p-3 rounded-xl border-2 border-slate-100 bg-white dark:bg-slate-800 flex items-center justify-between opacity-50 cursor-not-allowed">
                                                                        <span className="text-[10px] font-black text-slate-400 uppercase">{t.quiz.types?.trueFalse || "TRUE / FALSE"} ⚖️</span>
                                                                    </div>
                                                                    <div className={`p-3 rounded-xl border-2 flex items-center justify-between cursor-pointer transition-all ${allowedTypes.includes('multiple-choice') ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 hover:border-indigo-300'}`}>
                                                                        <span className={`text-[10px] font-black uppercase ${allowedTypes.includes('multiple-choice') ? 'text-indigo-700 dark:text-indigo-400' : 'text-slate-500'}`}>
                                                                            ● {t.quiz.types?.mcq || "MULTIPLE CHOICE"}
                                                                        </span>
                                                                        {allowedTypes.includes('multiple-choice') && <div className="w-4 h-4 rounded-full bg-slate-400/30 border-2 border-slate-600"></div>}
                                                                    </div>
                                                                    <div className="p-3 rounded-xl border-2 border-slate-100 bg-white dark:bg-slate-800 flex items-center justify-between opacity-50 cursor-not-allowed">
                                                                        <span className="text-[10px] font-black text-slate-400 uppercase">{t.quiz.types?.match || "MATCHING"} 🔗</span>
                                                                    </div>
                                                                    <div className="p-3 rounded-xl border-2 border-slate-100 bg-white dark:bg-slate-800 flex items-center justify-between opacity-50 cursor-not-allowed">
                                                                        <span className="text-[10px] font-black text-slate-400 uppercase">{t.quiz.types?.fillIn || "FILL IN BLANK"} 📝</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        )}
                                                            ...
                                                            <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto">
                                                                ...
                                                                {(currentQ.options && currentQ.options.length > 0 ? currentQ.options : bankTokens).map((token, idx) => (
                                                                    <button
                                                                        ...
                                                                        className="px-4 py-2 bg-white border-2 border-slate-200 shadow-[0_4px_0_0_rgba(226,232,240,1)] rounded-xl font-bold text-slate-600 hover:border-indigo-400 hover:text-indigo-600 hover:shadow-[0_4px_0_0_rgba(129,140,248,1)] active:translate-y-1 active:shadow-none transition-all"
                                                                    >
                                                                        {token}
                                                                    </button>
                                                                ))}
