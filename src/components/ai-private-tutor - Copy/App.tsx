...
import { Atom, DrawingAction, TeacherState, SessionConfig, NoteData, QuickAction } from './types';
...
interface AppProps {
  user?: { displayName?: string; firstName?: string; name?: string; };
}

export default function App({ user }: AppProps) {
  const [state, setState] = useState<TeacherState>(TeacherState.IDLE);
  const [mode, setMode] = useState<'screen' | 'pdf'>('screen');
  ...
  const liveService = useRef(new LiveSessionService());
  ...
  useEffect(() => {
    ...
    liveService.current.setCallbacks(
      (action) => {
        const enrichedAction = {
          ...action,
          scrollX: scrollRef.current.left,
          scrollY: scrollRef.current.top
        };
        setDrawingActions(prev => [...prev.slice(-15), enrichedAction]);
        setTimeout(() => {
          setDrawingActions(prev => prev.filter(a => a.id !== action.id));
        }, 12000);
      },
      (note) => {
      ...
      (newState) => {
        console.debug("App: State transition", newState);
        if (newState === 'SPEAKING') setState(TeacherState.EXPLAINING);
        if (newState === 'LISTENING') setState(TeacherState.LISTENING);

        // DISCONNECTED and generic ERROR are now handled as recoverable.
        // The UI will remain in its current state (likely LISTENING or EXPLAINING) 
        // while the service handles reconnection or waits for user intent.

        if (newState === 'ERROR_QUOTA') {
          setState(TeacherState.IDLE);
          alert("Connection Limit Reached (429). The system is busy or you need to use your own API Key.");
          setShowSettings(true);
        }
        else if (newState === 'ERROR_AUTH') {
          setState(TeacherState.IDLE);
          alert("Authentication Failed (403). Please check your API Key.");
          setShowSettings(true);
        }
        else if (newState === 'ERROR_UNSUPPORTED') {
          setState(TeacherState.IDLE);
          alert("Model/Region Not Supported. \n\nPlease switch to a paid US-based API Key.");
          setShowSettings(true);
        }
      },
      (newActions) => {
    ...
    // 🔒 Phase 1: Authority Observer (LISTENING/SPEAKING handled in onState)
  }, []);
  ...
  const handleStartSession = async () => {
    console.debug("App: handleStartSession", { sessionConfig });
    setShowSettings(false);
    if (!userApiKey) {
      alert("Please configure the API Key in environment variables to start.");
      setShowSettings(true);
      return;
    }
    try {
      console.debug("App: Connecting to Live Service...");
      await liveService.current.connect(sessionConfig, userApiKey);
      console.debug("App: Connection Successful. State -> LISTENING");
      setState(TeacherState.LISTENING);
    } catch (error: any) {
  ...
  const handleFrameCapture = useCallback((base64: string, metadata?: any) => {
    liveService.current.sendVideoFrame(base64, metadata); // Pass metadata
  }, []);
  ...
  const handleUserDraw = (action: DrawingAction) => {
    console.debug("App: handleUserDraw", action.type);
    ...
    <div className="flex flex-col md:flex-row h-screen bg-eduva-900 text-white overflow-hidden font-sans">
      ...
      <div className="hidden md:block">
        <CompactSidebar
          ...
          pdfFile={pdfFile}
          state={state}
          onToggleScreenShare={() => { if (state === TeacherState.IDLE) setMode('screen'); else handleToggleScreenShare(); }}
        ...
        <header className="h-12 border-b border-eduva-700 flex items-center justify-between px-4 bg-eduva-800/80 backdrop-blur-md z-10 relative shrink-0">
          <div className="flex items-center">
            ...
            <h2 className="text-lg font-medium text-slate-200 truncate max-w-[200px]">
              {mode === 'screen' ? (screenStream ? 'Live Analysis' : 'Voice Mode') : 'Study Mode'}
            </h2>
          </div>
        ...
        <div className="md:hidden fixed bottom-24 right-4 z-[60] flex flex-col items-end space-y-4">
          ...
          <div className={`flex flex-col items-end space-y-3 transition-all duration-300 origin-bottom ${mobileMenuState === 'none' ? 'opacity-0 scale-90 pointer-events-none' : 'opacity-100 scale-100'}`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => {
                  if (state === TeacherState.IDLE) setMode('screen'); else handleToggleScreenShare();
                  setMobileMenuState('none');
                }}
