...
export class LiveSessionService {
  ...
  private sendFirstTurnPayload(payload: { audioBase64?: string; text?: string }) {
    ...
    if (this.lastIdleVisionFrame) {
      // ⚛️ Send vision context using RELAY-COMPATIBLE 'media' key
      ...
      console.log(`[MULTIMODAL][FIRST_PAYLOAD] Buffered vision context sent id=${this._lastVisionFrameId}`);
      this.lastIdleVisionFrame = null;
      this.lastIdleVisionMetadata = null;
    }

    if (payload.text) {
      // ⚛️ Send text-turn trigger using RELAY-COMPATIBLE 'content' key
      this.safeSendRealtimeInput({
        content: [{ role: 'user', parts: [{ text: payload.text }] }]
      });
      console.log(`[MULTIMODAL][FIRST_PAYLOAD] RE-PROTOCOL (Content/Text) sent turnId=${this.currentTurnId} ts=${now}`);
    } else if (payload.audioBase64) {
      // ⚛️ Send audio chunk using RELAY-COMPATIBLE 'media' key
      this.safeSendRealtimeInput({
        media: {
      ...
      console.log(`[MULTIMODAL][FIRST_PAYLOAD] RE-PROTOCOL (Media/Audio) sent turnId=${this.currentTurnId} ts=${now}`);
    }

    // ⚛️ Send METADATA pulse using RELAY-COMPATIBLE 'content' key
    if (metadataParts.length > 0) {
      this.safeSendRealtimeInput({ content: [{ parts: metadataParts }] });
      console.log(`[MULTIMODAL][SECOND_PAYLOAD] PULSE_2 (Context) sent turnId=${this.currentTurnId} ts=${Date.now()}`);
    }
  ...
  private async processQueue() {
    ...
    while (this.messageQueue.length > 0) {
      ...
      if (msg.serverContent?.turnComplete) {
        ...
        if (!this.hasReceivedAudioForCurrentTurn && this.audioParts.length === 0) {
  ...
  private processAudioChunk(float32Data: Float32Array) {
    ...
    // Subsequent chunks using RELAY-COMPATIBLE 'media' key
    this.safeSendRealtimeInput({
      media: {
        mimeType: 'audio/pcm;rate=16000',
        data: base64Audio,
    ...
    for (const fc of msg.toolCall.functionCalls) {
      try {
        if (fc.name === 'drawOnScreen') {
          ...
          // --- AI Intent Validation (Phase 9) ---
          if (this.lastIdleVisionMetadata && this.lastIdleVisionMetadata.visibleRect) {
            const rect = this.lastIdleVisionMetadata.visibleRect;
            const buffer = 50; // Allow slight bleed over edges
            const xInRange = args.x >= (rect.x - buffer) && args.x <= (rect.x + rect.width + buffer);
            const yInRange = args.y >= (rect.y - buffer) && args.y <= (rect.y + rect.height + buffer);

            if (!xInRange || !yInRange) {
              console.warn(`[TURN AUTHORITY] AI drawing intent dropped (out of bounds): (${args.x}, ${args.y}) for visibleRect:`, rect);
              // For now, we allow it but log a warning. In strict mode, we would return.
            }
          }
          this.onDraw?.({ id: crypto.randomUUID(), type: args.actionType, ...args, timestamp: Date.now() });
        }
