...
export enum TeacherState {
  ...
  THINKING = 'THINKING',
  SPEAKING = 'SPEAKING',
  GENERATING = 'GENERATING', // New state for AI tool/text creation
  WATCHING = 'WATCHING', // Observing screen/PDF
  RECONNECTING = 'RECONNECTING', // 🔄 Internet is not stable
}

// --- Board Revamp Infrastructure ---
export type BoardLifecycle = 'idle' | 'loading' | 'ready' | 'detached';
export type BoardSource = 'pdf' | 'image' | 'screen' | 'board' | 'none';
...
export interface Stroke {
  id: string;
  author: 'user' | 'ai';
  tool: 'pen' | 'highlight' | 'arrow' | 'circle' | 'laser' | 'eraser' | 'text';
  path: { x: number; y: number }[]; // World Coordinates (PDF pixels @ scale=1)
  color: string;
  width: number;
  text?: string;
}
...
export interface BoardState {
  ...
  keepTeacherAnnotations: boolean;
}

export interface DrawingAction {
  id: string;
  type: 'circle' | 'arrow' | 'highlight' | 'rect' | 'freehand' | 'eraser' | 'text';
  x: number;
  y: number;
  width?: number;
  height?: number;
